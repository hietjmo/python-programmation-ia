...
ps = [Puncto (x1,y1),Puncto (x2,y1),Puncto (x2,y2),Puncto (x1,y2)]
# pg1 = Polygon ps
# hs = [Puncto (x1,y1),Puncto (x2,y1),Puncto (x1,y2)]
# pts3 = clip ([p.pair () for p in ps],[p.pair () for p in hs]) 
# pg3 = [Puncto (x,y) for [x,y] in pts3]
print (ps)
# draw_polygon (pg3)
...
diagonal1 = Linea (ps[0],ps[2])
diagonal2 = Linea (ps[1],ps[3])
...
print (diagonal1)
print (diagonal2)
center = intersection (diagonal1,diagonal2)
print (center)
for p in ps + [center]:
  draw_point (p)
